...
To compute the proportion of station entrances / exits without vending allow entrance, we first exclude station entrances that do not allow vending and then take the mean of the entry variable, which is logical, to produce the desired proportion:
```{r}
trans_ent %>% 
  filter(vending == "NO") %>% 
  pull(entry) %>% 
